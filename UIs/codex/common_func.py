...
def get_err_reason_of_folder_name_with_no_type(folder_name):
    if folder_name.startswith(Global_Varibles.prefix_of_folders):
        return f'系统分类文件夹'
    result = re.match(
        Global_Varibles.rough_regex_of_student_folder_with_no_type, folder_name)
    err_str = ""
    if result:
        match_dict = result.groupdict()
        if match_dict["campus"] not in Global_Varibles.All_Campus:  # 校区错误
            err_str = f'命名不合法的学生文件夹命名：  {folder_name }\n《错误原因：科中没有\"{match_dict["campus"]}\"校区》'
        elif match_dict["grade"] not in Global_Varibles.All_Grades:  # 年级错误
        ...
        elif int(match_dict["class_n"]) > Global_Varibles.campus_grade_class_count[match_dict["campus"]][match_dict["grade"]]:
            err_str = f'命名不合法的学生文件夹命名：  {folder_name }\n《错误原因：\"{match_dict["campus"]}-{match_dict["grade"]}\" 没有 \"{match_dict["class_n"]}班\"》'
        elif not re.match(Global_Varibles.regex_student_name, match_dict["name"]):
            err_str = f'命名不合法的学生文件夹命名：  {folder_name }\n《错误原因： 不合法的学生中文名： \"{match_dict["name"]}\"》'

        # print(re.match(Global_Varibles.regex_student_name, match_dict["name"]).group())
...
def get_err_reason_of_folder_name_with_type(folder_name):
    if folder_name.startswith(Global_Varibles.prefix_of_folders):
        return f'系统分类文件夹'
    result = re.match(
        Global_Varibles.rough_regex_of_student_folder_with_type, folder_name)
    if result:
        match_dict = result.groupdict()
        # 组成不含类型的命名
        ...
        if err_str == None:  # 基本信息正确
            if match_dict["type"] in Global_Varibles.All_Work_Types:  # 类型也正确
                return None
            else:
