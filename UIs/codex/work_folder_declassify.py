import os
from codex import Global_Varibles, dir_dealing
...
def delete_invalid_and_unranked_folder(path_work_root, ask_on_nonempty_folder=True):
    dir_dealing.delete_a_folder_with_warning(
        path_work_root+"\\"+Global_Varibles.Invalid_Folder_Name, ask_on_nonempty_folder=ask_on_nonempty_folder)
    dir_dealing.delete_a_folder_with_warning(
        path_work_root+"\\"+Global_Varibles.Unrankd_Folder_Name, ask_on_nonempty_folder=ask_on_nonempty_folder)
...
def declassify_all_student_work_folders(path_work_root, delete_empty_classified_folders=True):
    for root, dir, file in os.walk(path_work_root):
        if root.split("\\")[-1] in Global_Varibles.All_Classified_Folders_Name_List:  # 遍历所有归类文件夹
