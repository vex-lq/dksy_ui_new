...
# 注意，如果不合法，会返回错误原因字符串，正确才返回True
def is_legal_folder_name_without_type(folder_name):
    if folder_name.startswith(prefix_of_folders):
        return "系统分类文件夹"
    result = re.match(
        regex_str_of_student_folder_without_type, folder_name)
    err_str = ""
    if result:
...
def is_legal_folder_name_with_type(folder_name):
    if folder_name.startswith(prefix_of_folders):
        return "系统分类文件夹"
    result = re.match(
        regex_str_of_student_folder_with_type, folder_name)
    if result:
        match_dict = result.groupdict()
        # 组成不含类型的命名
        info_without_type_naming = match_dict["campus"]+"-" + \
            ...
            "班-"+match_dict["name"]
        err_str = is_legal_folder_name_without_type(info_without_type_naming)
        if err_str == True:
            if match_dict["type"] in All_Work_Types:
                return True
            else:
...
# print(is_legal_folder_name_with_type("清水河-高1-10班-发明-王虎"))
